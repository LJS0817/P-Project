...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        if(rows.length == 0) {
            return res.render('main/index', getUserContext(req,{
...
router.get('/delete/:id', async (req, res) => {
    ...
    try {
        ...
        const sql = `UPDATE projects SET is_deleted = 1 WHERE ${projectId == "all" ? '' : 'id = ? AND '}user_id = ?`;
        const result = await db.query(sql, projectId == "all" ? [userId] : [projectId, userId]);
