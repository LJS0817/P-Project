...
router.post("/signin", async (req, res) => {
  ...
  try {
    // DB에서 사용자 조회 (u.* 로 IS_ADMIN 컬럼도 같이 가져옴)
    ...
    if (rows.length > 0) {
      ...
      if (user.password === password) {
        // 비밀번호 일치
        console.log("로그인 성공 :", user.nickname, user.email, "Admin:", user.IS_ADMIN);

        // 세션에 사용자 정보 저장 (IS_ADMIN 추가)
        req.session.user = {
          id: user.id,
          login_id: user.login_id,
          nickname: user.nickname,
          email: user.email,
          IS_ADMIN: user.IS_ADMIN, // 관리자 권한 저장 (매우 중요)
...
router.get("/forgot-password", (req, res) => {
  console.log("현재 언어 설정:", req.getLocale());
  res.render(
    "auth/sign",
...
async function handleSocialLogin(
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      // login_id에 소셜 고유 ID 넣기 (IS_ADMIN 기본 0)
    ...
    // [핵심 수정] 세션 생성 (IS_ADMIN 추가)
    req.session.user = {
      id: user.id,
      login_id: user.login_id,
      nickname: user.nickname,
      email: user.email,
      IS_ADMIN: user.IS_ADMIN, // 관리자 권한 저장
      date_format: user.date_format || "YYYY. MM. DD",
      theme: user.theme || "default",
    };
