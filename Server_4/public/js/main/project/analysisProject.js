...
document.addEventListener('mousemove', (e) => {
    ...
    // 기존 썸네일 내부 아이콘 투명도 조절 (기존 코드 유지)
    if(thumb.children[2]) thumb.children[2].style.opacity = 1 - percentage;
    if(thumb.children[0]) thumb.children[0].style.opacity = 1 - percentage;

    // --- [추가된 기능] 이미지(.img.after) 투명도 조절 ---
    // 동적으로 생성된 요소이므로 이벤트 발생 시점에 찾아야 합니다.
    const targetImage = document.querySelector('.viewProject .result .img.after');
    
    if (targetImage) {
        // 슬라이더가 맨 위(percentage 1)일 때 히트맵(after)이 보이고(Opacity 1)
        // 슬라이더가 맨 아래(percentage 0)일 때 원본(before)만 보이게(Opacity 0) 설정
        targetImage.style.opacity = percentage;
    }
});
...
async function viewPage(ele) {
    ...
    try {
        ...
        if (data.success && data.analyzed && data.original && data.heatmap) {
            ...
            // 2. [추가] 슬라이더 및 이미지 투명도 초기화 (기본값 100%로 설정 예시)
            const initialPercentage = 1.0; // 1.0은 100% (최상단), 0.5는 50%
            const maxBottom = trackHeight - thumbHeight;
            const newBottom = maxBottom * initialPercentage;

            // 썸네일 위치 이동
            thumb.style.bottom = newBottom + 'px';
            
            // 이미지 투명도 즉시 적용
            const afterImg = dropArea.querySelector('.img.after');
            if(afterImg) afterImg.style.opacity = initialPercentage;

            // 썸네일 아이콘 투명도 초기화
            if(thumb.children[2]) thumb.children[2].style.opacity = 1 - initialPercentage;
            if(thumb.children[0]) thumb.children[0].style.opacity = 1 - initialPercentage;
        } else {
