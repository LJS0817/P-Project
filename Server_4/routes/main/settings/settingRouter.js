...
router.get('/', (req, res) => {
    res.render('main/index', getUserContext({content:"./settings/settings.ejs", title:"Settings", subtitle:"Manage your account Setting and preferences", which:"account", index: 0}), (e, content) => {
        console.log(e);
        res.end(content);
    });
});
...
router.get('/preferences', (req, res) => {
    res.render('main/index', getUserContext({content:"./settings/settings.ejs", title:"Settings", subtitle:"Manage your account Setting and preferences", which:"prefer", index: 1}), (e, content) => {
        res.end(content);
    });
});

router.get('/project', (req, res) => {
    res.render('main/index', getUserContext({content:"./settings/settings.ejs", title:"Settings", subtitle:"Manage your account Setting and preferences", which:"project", index: 2}), (e, content) => {
        res.end(content);
    });
});
...
router.post('/update', async (req, res) => {
    // 설정 업데이트 처리 로직 작성
    if (!req.session.user) {
        return res.status(401).send('로그인 안됨');
    }
    ...
    try {
        ...
        res.status(500).send({ success: false, message: 'DB 에러' });
    };
});
...
router.post('/update-account', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: 'DB가 아파요'});
    }
});
