...
router.post('/signin', async (req, res) => {
    ...
    try {
        ...
        if (rows.length > 0) { // 사용자가 존재 -> 비밀번호 비교
            const user = rows[0];
...
router.post('/signup', async (req, res) => {
    ...
    // DB에 사용자 추가
    try {
        const checkSql = 'SELECT login_id, email, nickname FROM users WHERE login_id = ? OR email = ? OR nickname = ?';
        const rows = await db.query(checkSql, [userid, useremail, finalNickname]);

        if (rows.length > 0) { // 중복된 데이터 존재시 중복 항목 확인
            const user = rows[0];
            let reason = [];
            let duplicateFields = [];
            ...
            let msg = 'Someone already has ' + duplicateFields.join(', ') + '. Try another';
...
async function handleSocialLogin(req, res, socialId, email, nickname, provider) {
    try {
        ...
        if (rows.length > 0) {
            ...
            // 설정 초기 값
            await db.query(`INSERT INTO user_settings (user_id) VALUES (?)`, [newUserId])
