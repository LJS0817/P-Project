class SmoothChart {
    ...
    resize() {
        ...
        // [추가됨] 미디어 쿼리 체크: 768px 이하일 때 높이와 패딩 조정
        if (window.matchMedia('(max-width: 768px)').matches) {
            this.height = 100;
            // 높이가 줄어든 만큼 상하 패딩도 줄여줍니다 (선택 사항)
            this.padding = { top: 40, right: 20, bottom: 35, left: 35 };
        } else {
            this.height = 200;
            // 원래 패딩 복구
        ...
        // 1. CSS 스타일 높이 강제 설정 (레이아웃 잡기)
        this.canvas.style.width = '100%';
        this.canvas.style.height = `${this.height}px`;

        // 2. 브라우저가 계산한 실제 픽셀 크기를 가져옴
        const rect = this.canvas.getBoundingClientRect();
        
        // 내부 논리 너비 업데이트
        this.width = rect.width;

        // 3. 캔버스 해상도(버퍼)는 DPR을 곱해서 선명하게 설정
        this.canvas.width = this.width * dpr;
        this.canvas.height = this.height * dpr;
        
        // Context 스케일 조정
        this.ctx.setTransform(1, 0, 0, 1, 0, 0); // 기존 변환 초기화
