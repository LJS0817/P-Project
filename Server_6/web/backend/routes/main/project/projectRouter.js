...
function formatDateTime(date, format) {
  if (!date) return "-";
  if (!format) format = "YYYY. MM. DD HH:mm"; // 기본값
...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    const projects = rows.map((row) => ({
      ...
      // [수정] 사용자 포맷 적용
      updated: formatDateTime(row.updated_at, userDateFormat),
...
router.get("/edit/:id", async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res.send(
        '<script>alert("존재하지 않거나 권한이 없음"); location.href="/dashboard/project";</script>'
      );
    }
...
router.post("/addProject", upload.single("thumbnail"), async (req, res) => {
  ...
  const thumbPath = req.file
    ? `/storage/uploads/${req.session.user.login_id}/${req.file.filename}`
    : "";
...
router.get("/view/:id", async (req, res) => {
  try {
    const projectId = req.params.id;
    const userDateFormat = req.session.user
    ...
    const pages = imageRows.map((img) => ({
      ...
      // [수정] 사용자 포맷 적용
      datetime: formatDateTime(img.created_at, userDateFormat),
    }));
    ...
    res.render(
      ...
      getUserContext(req, {
        ...
        title: req.__("dummy", project.title),
        subtitle: "projectViewSubTitle",
...
router.get("/delete/:id", async (req, res) => {
  ...
  try {
    ...
    const sql = `UPDATE projects SET is_deleted = 1 WHERE ${
      projectId == "all" ? "" : "id = ? AND "
    }user_id = ?`;
    const result = await db.query(
      sql,
      projectId == "all" ? [userId] : [projectId, userId]
    );
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        ...
        .json({
          success: false,
          message: "이미지를 찾을 수 없거나 권한이 없습니다.",
        });
    }
    ...
    res.status(200).json({
      ...
      heatmap: isAnalyzed ? data.heatmap_path : null,
      path: isAnalyzed ? data.path_path : null,
...
router.post("/page/create", async (req, res) => {
  ...
  if (!projectId || !newName) {
    return res
      .status(400)
      .json({ success: false, message: "필수 데이터(ID/이름) 누락" });
  }
  ...
  try {
    ...
    if (checkRows.length === 0) {
      return res
        ...
        .json({
          success: false,
          message: "권한이 없거나 존재하지 않는 프로젝트입니다.",
        });
    }
...
router.post("/page/rename", async (req, res) => {
  ...
  if (!imageId || !newName) {
    return res
      .status(400)
      .json({ success: false, message: "필수 데이터 미입력" });
  }
  ...
  try {
    ...
    if (result.affectedRows > 0) {
      ...
      res
        .status(403)
        .json({ success: false, message: "권한이 없거나 존재하지 않음" });
    }
  } catch (err) {
...
router.post("/analyze", upload.single("fileData"), async (req, res) => {
  ...
  const imageIdInput = req.body.imageId;
  // const originalName = Buffer.from(req.file.originalname, "latin1").toString(
  //   "utf-8"
  // ); // 원본 파일 이름
  // const originalName = req.body.pageName;
  ...
  // let pathOutput = path.join(resultDir, "path_" + req.file.filename)
...
let imageId;
  try {
    ...
    // [수정 2] UPDATE 실행
    const result = await db.query(updateImageSql, [
      storedName,
      webOriginalPath,
      req.file.size,
      imageIdInput     // WHERE 절에 들어갈 ID
    ]);

    // 업데이트된 행이 없는 경우 (ID가 잘못됨)
    if (result.affectedRows === 0) {
        return res.status(404).json({ success: false, message: "해당 이미지를 찾을 수 없습니다." });
    }

    imageId = imageIdInput; // 뒤쪽 로직(Python 실행, 결과 저장 등)에서 사용하기 위해 할당

  } catch (err) {
    console.error("DB 이미지 업데이트 실패", err);
  ...
  pyProcess.on("close", async (code) => {
    ...
    if (code === 0) {
      ...
      console.log("Python 분석 결과:", resultJson);
      const scanpathFileName = path.basename(resultJson.output_scanpath);
      const webScanpathPath = `/storage/results/${userId}/${scanpathFileName}`;

      // 성공 시 : 웹에서 접근 가능한 이미지 URL 반환
      await db.query(`UPDATE images SET status = '완료' WHERE id = ?`, [
        imageId,
      ]); // images 테이블 상태 업데이트

        await db.query(`DELETE FROM analysis_results WHERE image_id = ?`, [imageId]);

        // 3-2. 새로운 분석 결과 삽입
        await db.query(
          `
          INSERT INTO analysis_results (image_id, heatmap_path, path_path, balance_score, focus_score)
          VALUES (?, ?, ?, ?, ?)
          `,
          [imageId, webHeatmapPath, webScanpathPath, resultJson.scores.balance_score, resultJson.scores.focus_score]
        );

      // const webImageUrl = `/storage/results/${userId}/${outputFileName}`;

    console.log(resultJson.scores); 
      res.status(200).json({
        success: true,
        message: "분석 완료",
        original: webOriginalPath, // 원본 경로
        heatmap: webHeatmapPath, // 결과 히트맵 경로
        path: webScanpathPath,
        scores: {
              balance: resultJson.scores.balance_score,
              focus: resultJson.scores.focus_score
          }
      });
    } else {
      // 실패 시 업데이트
      await db.query(
        `UPDATE images SET status= '실패', failure_message = ? WHERE id = ?`,
