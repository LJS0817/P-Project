...
const getTheme = ["default", "juice", "sand", "lavender", "jungle"];
...
themeModes.forEach((mode, index) => {
  mode.addEventListener("click", () => {
    ...
    if (index == 1 && !document.body.classList.contains("dark"))
      document.body.classList.add(
        "dark"
      ); // index == 1 (dark) body에 dark 클래스가 없다면 추가
    else if (document.body.classList.contains("dark"))
      document.body.classList.remove(
        "dark"
      ); // 이미 dark 클래스가 있다면 dark 제거
    else if (
      index == 2 &&
      window.matchMedia &&
      window.matchMedia("(prefers-color-scheme: dark)").matches
    ) {
      // 시스템 설정이 다크모드라면 dark 클래스 추가
...
if (saveBtn) {
  saveBtn.addEventListener("click", (e) => {
    ...
    const timeToggle = document.getElementById("themeToggle");

    if (!dateFormatEl || !themeEl || !languageEl) {
      console.log("요소 없음");
      return;
    }

    let dateFormatValue = dateFormatEl.value;
    const themeValue = themeEl.value;
    const languageValue = languageEl.value;

    if (timeToggle && timeToggle.checked) {
      dateFormatValue += " HH:mm";
    } else {
      dateFormatValue += " a hh:mm";
    }

    window.openPopup(
      document.textData["setting_preferSavePopupTitle"],
      "",
      `<p>${document.textData["setting_preferSavePopupMsg"]}<i class="fi fi-tr-it"></i></p>`,
      () => {
        window.sendData(
